...
if [ -z "${FILES_DIR+x}" ]; then
	if [ -f "${SCRIPT_DIR}/${IMAGE_BASENAME}_boot.img" ]; then
...
check_fits() {
	...
	size=$(file_size "$file")
	[ "$size" -le "$limit" ] || {
		echo "$file is $size bytes, larger than partition limit $limit bytes" >&2
		exit 1
	}
}

for c in dd mkdir sfdisk sgdisk sha256sum truncate wc xz; do
	need_cmd "$c"
done
...
FULL_IMG="${FILES_DIR}/${IMAGE_BASENAME}.img"
FULL_XZ="${FILES_DIR}/${IMAGE_BASENAME}.img.xz"
GPT_TXT="${FILES_DIR}/${IMAGE_BASENAME}_gpt.txt"
BOOT_IMG="${FILES_DIR}/${IMAGE_BASENAME}_boot.img"
ROOTFS_IMG="${FILES_DIR}/${IMAGE_BASENAME}_rootfs.img"
...
check_fits "${FILES_DIR}/sbl1.mbn" $((SBL1_SIZE * 512))
check_fits "${FILES_DIR}/rpm.mbn" $((RPM_SIZE * 512))
check_fits "${FILES_DIR}/tz.mbn" $((TZ_SIZE * 512))
check_fits "${FILES_DIR}/hyp.mbn" $((HYP_SIZE * 512))
check_fits "${FILES_DIR}/aboot.mbn" $((ABOOT_SIZE * 512))
check_fits "$BOOT_IMG" $((BOOT_SIZE * 512))
check_fits "$ROOTFS_IMG" $((ROOTFS_SIZE * 512))
...
dd if="${FILES_DIR}/sbl1.mbn" of="$FULL_IMG" bs=512 seek=$SBL1_START conv=notrunc status=none
dd if="${FILES_DIR}/rpm.mbn" of="$FULL_IMG" bs=512 seek=$RPM_START conv=notrunc status=none
dd if="${FILES_DIR}/tz.mbn" of="$FULL_IMG" bs=512 seek=$TZ_START conv=notrunc status=none
dd if="${FILES_DIR}/hyp.mbn" of="$FULL_IMG" bs=512 seek=$HYP_START conv=notrunc status=none
...
dd if="${FILES_DIR}/aboot.mbn" of="$FULL_IMG" bs=512 seek=$ABOOT_START conv=notrunc status=none
dd if="$BOOT_IMG" of="$FULL_IMG" bs=512 seek=$BOOT_START conv=notrunc status=none
dd if=/dev/zero of="$FULL_IMG" bs=512 seek=$DEVINFO_START count=$DEVINFO_SIZE conv=notrunc status=none
dd if="$ROOTFS_IMG" of="$FULL_IMG" bs=512 seek=$ROOTFS_START conv=notrunc status=none
